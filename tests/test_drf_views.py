...
class TrackDownloadAPIViewTest(TestCase):
    def setUp(self):
        cache.clear()
    ...
    def test_tract_download_api_success(self, mock_get_geolocation, mock_get_client_ip):
        self.assertEqual(self.target.clicks.count(), 0)
        url = reverse('clickify-drf:track_download_api',
    ...
    def test_track_download_api_nonexistent_slug(self, mock_get_geolocation, mock_get_client_ip):
        url = reverse('clickify-drf:track_download_api',
    ...
    def test_track_download_api_rate_limit_exceeded(self, mock_get_geolocation, mock_get_client_it):
        url = reverse('clickify-drf:track_download_api',
                      kwargs={'slug': self.target.slug})
